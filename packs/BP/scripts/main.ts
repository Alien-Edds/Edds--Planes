import { EntityHealthComponent, MinecraftDimensionTypes, Player, system, world } from "@minecraft/server"
...
world.afterEvents.entityHitEntity.subscribe((data) => {
    ...
    const player = data.damagingEntity as Player
    if (!player.isSneaking) return
    if (!plane.isValid()) return
    const healthComp = plane.getComponent(EntityHealthComponent.componentId) as EntityHealthComponent
